...
#将每个考场尽最大量填充
#公务员、警察、选调生分别执行脚本
#数据库表结构ksinfo:ksxm/zkzh/sfzh/bkdw/bkzw/leixing
#            zhiwei:dwmc/zwmc/leixing/rs/kcxh
...
def Initzhiwei(zhiweilist):
    #这里从数据库读取职位形成列表
    query="SELECT bkdw,bkzw,leixing,count(*) FROM ksinfo where leixing='公务员' group by bkdw,bkzw ;" 
    cur.execute(query)
    zw=cur.fetchall()
...
def arrange_kc(zhiweilist):
    ...
    while len(zhiweilist)!=0:#结束条件为职位列表为空，即所有职位均分配到考场中
        ...
        for item in sol:
            ...
            query="insert into zhiwei (`dwmc`, `zwmc`, `leixing`, `rs`, `kcxh`) values('%s','%s','%s',%s,%s)" % (zhiweilist[item].dwmc,zhiweilist[item].zwmc,zhiweilist[item].leixing,zhiweilist[item].rs,i+1)
            cur.execute(query)
            print query
...
def sol_terminal():
    ...
    query='SELECT ksinfo.*,zhiwei.kcxh FROM ksinfo,zhiwei where ksinfo.bkdw=zhiwei.dwmc and ksinfo.bkzw=zhiwei.zwmc order by kcxh;'
    cur.execute(query)
    info=cur.fetchall()
    for item in info:
...
if __name__ == "__main__":
    zhiweilist=[]
    Initzhiwei(zhiweilist)
    arrange_kc(zhiweilist)
    #sol_terminal()
